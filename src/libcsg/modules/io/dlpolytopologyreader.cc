...
// the integer (anything after the first integer is ignored)
{
  ...
  unsigned int i = 1;
  do { // find integer number in the field with the lowest index (closest to the
       // keyword)
...
bool DLPOLYTopologyReader::ReadTopology(string file, Topology &top) {
  ...
  if (boost::filesystem::basename(filepath).size() == 0) {
    if (filepath.parent_path().string().size() == 0) {
      _fname =
          "FIELD"; // DL_POLY uses fixed file names in current/working directory
    } else {
  ...
  if (!fl.is_open()) {
    ...
    if (line.substr(0, 4) ==
        "NEUT") { // skip 'neutral groups' line (DL_POLY Classic FIELD format)
    ...
    for (int nmol_type = 0; nmol_type < nmol_types; nmol_type++) {
      ...
      for (int i = 0; i < natoms;) { // i is altered in repeater loop
        ...
        for (int j = 0; j < repeater; j++) {
          ...
          auto bead =
              top.CreateBead(1, beadname, type, res->getId(), mass, charge);
        ...
        if ((line == "BONDS") || (line == "ANGLES") || (line == "DIHEDR")) {
          ...
          for (int i = 0; i < count; i++) {
            ...
            if (type == "BONDS") {
              ...
              sl >> ids[2];
              sl >> ids[3];
              ic = shared_ptr<IDihedral>(new IDihedral(
                  id_map[ids[0] - 1], id_map[ids[1] - 1], id_map[ids[2] - 1],
