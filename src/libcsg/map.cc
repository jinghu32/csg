...
void Map_Sphere::Initialize(shared_ptr<Molecule> in, shared_ptr<Bead> out,
  ...
  for (size_t i = 0; i < beads.size(); ++i) {
    ...
    if (bead_ids.size() == 0) {
      ...
      string err = "impossible to resolve correct bead from name " + beads[i] +
                   " as more than one bead has the name.";
    }
    auto bead = in->getBead<Bead>(bead_ids.at(0));
