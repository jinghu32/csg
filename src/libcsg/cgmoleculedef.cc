...
shared_ptr<Molecule> CGMoleculeDef::CreateMolecule(Topology &top) {
  ...
  for (auto bead : _beads) {
    ...
    auto beadnew =
        top.CreateBead(bead->_symmetry, bead->_name, bt, res->getId(), 0, 0);
...
Map *CGMoleculeDef::CreateMap(shared_ptr<Molecule> in,
                              shared_ptr<Molecule> out) {
  ...
  for (vector<beaddef_t *>::iterator def = _beads.begin(); def != _beads.end();
    ...
    if (bead_ids.size() == 0) {
      ...
      string err = "Impossible to resolve single bead with name " +
                   (*def)->_name +
                   " as there exist more than a single bead with that "
                   "name.";
      throw runtime_error(err);
    }
